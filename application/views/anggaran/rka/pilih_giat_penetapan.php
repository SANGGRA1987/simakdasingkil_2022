...
//});
	  ...
	  //$(function(){
		$('#giat').combogrid({  
            ...
            mode:'remote',
            url:'<?php echo base_url(); ?>/index.php/rka/ld_giat_rancang/'+''+'/'+'',  
            columns:[[  
...
/* 		  $(function(){
        ...
        function validate_giat(skpd,urusan){
		 // $(function(){
			  $("#st_lintas").is(':checked');
			

			//saat pilih kegiatan
			$('#giat').combogrid({  
            url:'<?php echo base_url(); ?>/index.php/rka/ld_giat_rancang/'+skpd+'/'+urusan
            });
	...
	$("#dg").edatagrid("selectAll");
        ...
        //alert(skpd+urusan);
            $(function(){
			$('#dg').edatagrid({
				url: '<?php echo base_url(); ?>/index.php/rka/select_giat_rancang/'+skpd,
                 idField:'id',
                 toolbar:"#toolbar",              
                 rownumbers:"true", 
                 fitColumns:"true",
	...
	$(function(){
            ...
            //alert(skpd+urusan);
			$('#dg').edatagrid({
				url: '<?php echo base_url(); ?>/index.php/rka/select_giat_rancang',
                 idField:'id',
                 toolbar:"#toolbar",              
                 rownumbers:"true", 
                 fitColumns:"true",
		...
		$(document).ready(function(){
               // alert(csql);
                $.ajax({
                    type: "POST",   
                    dataType : 'json',                 
                    data: ({no:kode,tabel:'trskpd',field:'kd_kegiatan1'}),
                    url: '<?php echo base_url(); ?>/index.php/rka/cek_data',
                    success:function(data){                        
                        status_cek = data.pesan;
						if(status_cek==1 ){
						alert("Kode Kegiatan Telah Dipakai!");
        ...
        //alert(urusan+skpd+baru+lama+jns);
            $(function(){
				$('#dg').edatagrid({
				     url: '<?php echo base_url(); ?>/index.php/rka/psimpan_rancang/'+skpd+'/'+urusan+'/'+baru+'/'+lama+'/'+$.trim(jns)+'/'+lanjut,
					 idField:'id',
					 toolbar:"#toolbar",              
					 rownumbers:"true", 
					 fitColumns:"true",
        ...
        function hapus(){
               ...
               // alert(csql);
                $.ajax({
                    type: "POST",   
                    dataType : 'json',                 
                    data: ({no:rek,tabel:'trdrka',field:'kd_kegiatan'}),
                    url: '<?php echo base_url(); ?>/index.php/rka/cek_data',
                    success:function(data){                        
                        status_cek = data.pesan;
						if(status_cek==1 ){
						alert("Kode Kegiatan Telah Diturunkan di menu Pilih Program Kegiatan !");
