...
/* 		  $(function(){
	...
	$("#dg").edatagrid("selectAll");
        ...
        //alert(skpd+urusan);
            $(function(){
			$('#dg').edatagrid({
				url: '<?php echo base_url(); ?>/index.php/rka/ambil_data_taspen/'+cbln,
                 idField:'id',
                 toolbar:"#toolbar",              
                 rownumbers:"true", 
                 fit:"true",
                 ...
                 columns:[[
					...
					{field:'KDSKPD_SIM',title:'KDSKPD_SIM',width:80,align:'left'},
                    {field:'NMSKPD',title:'NMSKPD',width:500,align:'left'},                    
					{field:'NO_SPD',title:'NO_SPD',width:55},
                    {field:'TGL_SPD',title:'TGL_SPD',width:65,align:'center'},
                    {field:'NO_SPP',title:'NO_SPP',width:55,align:'center'},
                    {field:'TGL_SPP',title:'TGL_SPP',width:65,align:'center'},
                    {field:'NO_SPM',title:'NO_SPM',width:55,align:'center'},
                    {field:'TGL_SPM',title:'TGL_SPM',width:65,align:'center'},
                    {field:'NO_SP2D',title:'NO_SP2D',width:55,align:'center'},
                    {field:'TGL_SP2D',title:'TGL_SP2D',width:65,align:'center'},
                    {field:'KETERANGAN',title:'KETERANGAN',width:200,align:'center'},
                    {field:'GAPOK',title:'GAPOK',width:100,align:'right'},
                    {field:'TJKEL',title:'TJKEL',width:100,align:'right'},
                    {field:'TJSTRUK',title:'TJSTRUK',width:100,align:'right'},
                    {field:'TJFUNGSI',title:'TJFUNGSI',width:100,align:'right'},
                    {field:'TJUMUM',title:'TJUMUM',width:100,align:'right'},
                    {field:'TJBERAS',title:'TJBERAS',width:100,align:'right'},
                    {field:'TJPAJAK',title:'TJPAJAK',width:100,align:'right'},
                    {field:'TBULAT',title:'TBULAT',width:100,align:'right'},
                    {field:'TJKHUSUS',title:'TJKHUSUS',width:100,align:'right'},
                    {field:'PTAPERUM',title:'PTAPERUM',width:100,align:'right'},
                    {field:'PIWP',title:'PIWP',width:100,align:'right'},
                    {field:'PIWP2',title:'PIWP2',width:100,align:'right'},
                    {field:'PIWP8',title:'PIWP8',width:100,align:'right'},
                    {field:'PHUTANG',title:'PHUTANG',width:100,align:'right'},
                    {field:'PPAJAK',title:'PPAJAK',width:100,align:'right'},
                    {field:'TGLGAJI',title:'TGLGAJI',width:80,align:'right'},
                    {field:'KDJNSTRANS',title:'KDJNSTRANS',width:80,align:'right'},
                    {field:'KDSKPD',title:'KDSKPD',width:80,align:'right'},
                    {field:'JNSGAJI',title:'JNSGAJI',width:80,align:'right'},
				]]
			});
		});
        }
	...
	$(function(){
        ...
        function simpan(){
			...
			for(var i=0;i<rows.length;i++){            
				cidx        = rows[i].id;
                ckdskpdsim  = rows[i].KDSKPD_SIM.trim();
				cnospd      = rows[i].NO_SPD;
				ctglspd     = rows[i].TGL_SPD;
				cnospp      = rows[i].NO_SPP;
				ctglspp     = rows[i].TGL_SPP;
				cnospm      = rows[i].NO_SPM;
				ctglspm     = rows[i].TGL_SPM;
				cnosp2d     = rows[i].NO_SP2D;
				ctglsp2d    = rows[i].TGL_SP2D;
				cket        = rows[i].KETERANGAN.trim();
	            cgapok      = angka(rows[i].GAPOK);
                ctjkel      = angka(rows[i].TJKEL);
                ctjstruk    = angka(rows[i].TJSTRUK);
                ctjfungsi   = angka(rows[i].TJFUNGSI);
                ctjumum     = angka(rows[i].TJUMUM);
                ctjberas    = angka(rows[i].TJBERAS);
                ctjpajak    = angka(rows[i].TJPAJAK);
                ctbulat     = angka(rows[i].TBULAT);
                ctjkhusus   = angka(rows[i].TJKHUSUS);   	
    			cptaperum   = angka(rows[i].PTAPERUM);
                cpiwp       = angka(rows[i].PIWP);
                cpiwp2      = angka(rows[i].PIWP2);
                cpiwp8      = angka(rows[i].PIWP8);
                cphutang    = angka(rows[i].PHUTANG);
                cppajak     = angka(rows[i].PPAJAK);
                ctglgaji    = rows[i].TGLGAJI;
                ckdjnstrans = rows[i].KDJNSTRANS;
                cnmskpd     = rows[i].NMSKPD.trim();
                ckdskpd     = rows[i].KDSKPD.trim();
                cjnsgaji    = rows[i].JNSGAJI;
				if(i>0){
					csql = csql+","+"('"+ckdskpdsim+"','"+cnospd+"','"+ctglspd+"','"+cnospp+"','"+ctglspp+"','"+cnospm+"','"+ctglspm+"',"+
                            "'"+cnosp2d+"','"+ctglsp2d+"','"+cket+"','"+cgapok+"','"+ctjkel+"','"+ctjstruk+"','"+ctjfungsi+"','"+ctjumum+"','"+ctjberas+"','"+ctjpajak+"',"+
                            "'"+ctbulat+"','"+ctjkhusus+"','"+cptaperum+"','"+cpiwp+"','"+cpiwp2+"','"+cpiwp8+"','"+cphutang+"','"+cppajak+"','"+ctglgaji+"',"+
                            "'"+ckdjnstrans+"','"+cnmskpd+"','"+ckdskpd+"','"+cjnsgaji+"')";
				}else{
					csql = "values('"+ckdskpdsim+"','"+cnospd+"','"+ctglspd+"','"+cnospp+"','"+ctglspp+"','"+cnospm+"','"+ctglspm+"',"+
                            "'"+cnosp2d+"','"+ctglsp2d+"','"+cket+"','"+cgapok+"','"+ctjkel+"','"+ctjstruk+"','"+ctjfungsi+"','"+ctjumum+"','"+ctjberas+"','"+ctjpajak+"',"+
                            "'"+ctbulat+"','"+ctjkhusus+"','"+cptaperum+"','"+cpiwp+"','"+cpiwp2+"','"+cpiwp8+"','"+cphutang+"','"+cppajak+"','"+ctglgaji+"',"+
                            "'"+ckdjnstrans+"','"+cnmskpd+"','"+ckdskpd+"','"+cjnsgaji+"')";                 
				}                                             
			...
			$(document).ready(function(){
				...
				$.ajax({
					type: "POST",   
					dataType : 'json',                 
					data: ({sql:csql,cbln:cbln}),
					url: '<?php echo base_url(); ?>/index.php/rka/dsimpan_taspen',
					success:function(data){                        
						status = data.pesan;   
						 if (status=='1'){
...
<div id="content">   
 <!-- <?php echo $prev; ?>-->
  ...
  <h3>K E G I A T A N &nbsp;&nbsp;&nbsp;&nbsp;<input id="giat" name="giat" style="width: 150px;" />&nbsp;&nbsp;&nbsp;<input id="nm_giat" name="nm_giat" readonly="true" style="width:650px;border: 0;"/> </h3>-->
   <h3>&nbsp;</h3>
   <h3>BULAN&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<select  name="kebutuhan_bulan" id="kebutuhan_bulan" >
   ...
   &nbsp;&nbsp;&nbsp;&nbsp; <button type="button" onclick="javascript:get_taspen()">Tampil Taspen</button>&nbsp;&nbsp;<button type="button" onclick="javascript:simpan()">Simpan Data</button></h3> 
   <table id="dg" title="Data Taspen" style="width:910%;height:300%" >  
