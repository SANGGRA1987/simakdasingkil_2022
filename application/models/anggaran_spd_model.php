...
class anggaran_spd_model extends CI_Model {
    ...
    function jumlah_detail_angkas_spd_baru_asl($skp,$jn){ /*cek selisih angkas*/
    ...
    function jumlah_detail_angkas_spd_baru($skp,$jn){ /*cek selisih angkas*/
         $n_status = $this->rka_model->status_anggaran($skp);
         $nilai = 'a.'.$n_status;
         // print_r($n_status);die();
			 // $sql = "SELECT count(*) as total from (
    //            select kd_subkegiatan,kd_rek6,sum(nilai) as nilai,sum(angkas) as angkas,
    //            (sum(nilai))-(sum(angkas)) as total from(
    //            select a.kd_subkegiatan,a.kd_rek6,sum($n_status) as nilai,0 as angkas from trdrka a
    //            inner join (select kd_subkegiatan from trdskpd_ro GROUP BY kd_subkegiatan) b 
    //            on a.kd_subkegiatan=b.kd_subkegiatan where a.kd_skpd='$skp' AND LEFT(kd_rek6,1)= '5' group by a.kd_subkegiatan,a.kd_rek6 
    //            union all
    //            select a.kd_subkegiatan,a.kd_rek6,0 as nilai,sum(nilai) as angkas from trdskpd_ro a inner join
    //            (select kd_subkegiatan kd_subkegiatan from trdrka GROUP BY kd_subkegiatan) b 
    //            on a.kd_subkegiatan=b.kd_subkegiatan where a.kd_skpd='$skp' group by a.kd_subkegiatan,a.kd_rek6 
    //            )z group by kd_subkegiatan,kd_rek6
    //        )x where x.total != 0 ";

        $sql = "SELECT a.kd_skpd, a.nm_skpd, SUM( $nilai ) AS anggaran,(SELECT SUM($n_status) from trdskpd_ro WHERE kd_skpd = a.kd_skpd AND LEFT ( kd_rek6, 1 ) = '5')   AS angkas
                FROM trdrka a WHERE a.kd_skpd = '$skp' AND LEFT ( a.kd_rek6, 1 ) = '5' GROUP BY a.kd_skpd, a.nm_skpd";
        $query1 = $this->db->query($sql);        
        foreach($query1->result_array() as $resulte){ 
            $ang     = $resulte['anggaran'];
            $angkas  = $resulte['angkas'];
            $selisih = $ang - $angkas;
   ...
   /*function config_spd_nomor(){    
    ...
    function load_spd_bl_angkas_asl($kd_skpd,$page, $rows,$offset,$kriteria,$id,$beban) {
        if($beban=='belanja'){
            $beban=5;
            $judul="BELANJA";
        }else{
    ...
    function load_tot_dspd_bl_asl($jenis,$skpd,$awal,$ahir,$nospd,$tgl1){
    ...
    function load_tot_dspd_bl($jenis='',$skpd='',$awal='',$ahir='',$nospd='',$tgl1=''){
        ...
        */
        ...
        $spd = str_replace('123456789','/',$nospd);
        $sql = "SELECT SUM(nilai) as nilai FROM
                (SELECT a.kd_subkegiatan, a.nm_subkegiatan, a.kd_program, a.nm_program, '' as kd_rek6 , '' as nm_rek6, 
                 kd_skpd
                FROM trskpd a WHERE a.kd_skpd='$skpd' and a.jns_kegiatan ='5') a
                LEFT JOIN
                (SELECT kd_subkegiatan, kd_skpd, SUM($n_status) as nilai FROM $n_trdskpd b WHERE b.bulan>='$awal' AND b.bulan<='$ahir'
                AND kd_skpd='$skpd'
                GROUP BY kd_subkegiatan, kd_skpd)b
                ON a.kd_subkegiatan=b.kd_subkegiatan AND a.kd_skpd=b.kd_skpd";
        $query1 = $this->db->query($sql);  
        $ii     = 0;
    ...
    function load_dspd_bl_asl($jenis,$skpd,$awal,$ahir,$nospd,$cbln1,$tgl,$page,$rows,$offset,$kriteria){
    ...
    function load_dspd_bl($jenis,$skpd,$awal,$ahir,$nospd,$cbln1,$tgl,$page,$rows,$offset,$kriteria){
        ...
        $sql = "SELECT a.kd_subkegiatan, a.nm_subkegiatan, a.kd_program, a.nm_program, '' as kd_rek5 , '' as nm_rek5, 
                a.total_ubah as anggaran, nilai,lalu FROM(
                   
                select a.kd_subkegiatan , b.nm_subkegiatan, b.kd_program, b.nm_program, '' as kd_rek5 , '' as nm_rek5, 
                sum(a.$n_status) as total_ubah, left(a.kd_skpd,17) kd_skpd
                FROM trdrka a inner join trskpd b on a.kd_subkegiatan=b.kd_subkegiatan
                WHERE left(a.kd_skpd,17)='$dskpd' and left(a.kd_rek6,1) ='$jns_giat' 
                group by a.kd_subkegiatan, b.nm_subkegiatan, b.kd_program, b.nm_program,a.kd_skpd

                ) a LEFT JOIN (

                   SELECT kd_subkegiatan , LEFT(kd_skpd, 17) as kd_skpd, SUM ($n_status) AS nilai FROM trdskpd_ro
                   WHERE LEFT (kd_skpd, 17) = '$dskpd' and left(kd_rek6,1)='$jns_giat' AND bulan BETWEEN '$awal' AND '$ahir' 
                   GROUP BY kd_subkegiatan, LEFT (kd_skpd, 17)

                )b ON a.kd_subkegiatan=b.kd_subkegiatan AND left(a.kd_skpd,17)=left(b.kd_skpd,17) 
                LEFT JOIN (
                    SELECT kd_subkegiatan,SUM(a.nilai_final) as lalu FROM trdspd a LEFT JOIN trhspd b ON a.no_spd=b.no_spd 
                    WHERE left(b.kd_skpd,17)='$dskpd' and a.no_spd != '$spd' and b.jns_beban='$jns_giat' and bulan_akhir <'$cbln1'
                    GROUP BY kd_subkegiatan
                ) c ON a.kd_subkegiatan=c.kd_subkegiatan              
...
//tabel:'trhspd',field:'no_spd',no:cno,awal:bln1,akhir:bln2,skpd:skpd
