...
class Tukd_model extends CI_Model {
    ...
    function status_anggaran($skpd)
    {
        ...
        $query = $this->db->query($ang);
        foreach ($query->result() as $sts_ang) {
        	$status = $sts_ang->anggaran;
        }
        return $status;
    }
	...
	function _mpdf_F4($judul='',$isi='',$lMargin='',$rMargin='',$font='',$orientasi='',$hal='', $fonsize='') {
	...
	function _mpdf($judul='',$isi='',$lMargin='',$rMargin='',$font=0,$orientasi='') {
        ...
        $jam = date("H:i:s");
        $this->mpdf->SetFooter('Printed on @ {DATE j-m-Y} |Simakda| Halaman {PAGENO} dari {nb}');
...
//        }
    ...
    function  dotrek($rek){
        		...
        		break;
                case 8:
                    $rek = $this->left($rek,1).'.'.substr($rek,1,2).'.'.substr($rek,3,2).'.'.substr($rek,5,1).'.'.substr($rek,6,2);                             
                break;
                case 10:
                    $rek = $this->left($rek,1).'.'.substr($rek,1,2).'.'.substr($rek,3,2).'.'.substr($rek,5,1).'.'.substr($rek,6,2).'.'.substr($rek,8,2);                             
                break;
                case 12:
