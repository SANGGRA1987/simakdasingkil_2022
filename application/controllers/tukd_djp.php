...
(select REPLACE(nm_urusan,',','') from dbo.ms_urusan where kd_urusan=SUBSTRING(d.kd_skpd,1,4)) else
(select REPLACE(nm_urusan,',','') from dbo.ms_urusan where kd_urusan=SUBSTRING(d.kd_skpd,1,4)) end AS URAIANURUSAN,
CASE WHEN SUBSTRING(d.kd_skpd,1,4) is null then SUBSTRING(d.kd_skpd,1,4) else SUBSTRING(d.kd_skpd,1,4) end AS KDKELURUSAN,
CASE WHEN SUBSTRING(d.kd_skpd,1,4) is null then 
(select REPLACE(nm_urusan,',','') from dbo.ms_urusan where kd_urusan=SUBSTRING(d.kd_skpd,1,4)) else
(select REPLACE(nm_urusan,',','') from dbo.ms_urusan where kd_urusan=SUBSTRING(d.kd_skpd,1,4)) end AS URAIANKELURUSAN,
CASE WHEN SUBSTRING(d.kd_skpd,6,2) is null then SUBSTRING(d.kd_skpd,6,2) else SUBSTRING(d.kd_skpd,6,2) end AS KDORG,
(select REPLACE(nm_org,',','') from ms_organisasi where kd_org=left(d.kd_skpd,7)) AS URAIANORG,
...
RTRIM(REPLACE(REPLACE(REPLACE(d.keperluan, CHAR(13), ''),CHAR(10),''),',','')) KET,
...
(select REPLACE(nm_urusan,',','') from dbo.ms_urusan where kd_urusan=SUBSTRING(a.kd_skpd,1,4)) else
(select REPLACE(nm_urusan,',','') from dbo.ms_urusan where kd_urusan=SUBSTRING(a.kd_skpd,1,4)) end AS URAIANURUSAN,
CASE WHEN SUBSTRING(a.kd_skpd,1,4) is null then SUBSTRING(a.kd_skpd,1,4) else SUBSTRING(a.kd_skpd,1,4) end AS KDKELURUSAN,
CASE WHEN SUBSTRING(a.kd_skpd,1,4) is null then 
(select REPLACE(nm_urusan,',','') from dbo.ms_urusan where kd_urusan=SUBSTRING(a.kd_skpd,1,4)) else
(select REPLACE(nm_urusan,',','') from dbo.ms_urusan where kd_urusan=SUBSTRING(a.kd_skpd,1,4)) end AS URAIANKELURUSAN,
CASE WHEN SUBSTRING(a.kd_skpd,6,2) is null then SUBSTRING(a.kd_skpd,6,2) else SUBSTRING(a.kd_skpd,6,2) end AS KDORG,
(select REPLACE(nm_org,',','') from ms_organisasi where kd_org=left(a.kd_skpd,7)) AS URAIANORG,
...
RTRIM(REPLACE(REPLACE(REPLACE(a.keterangan+'(SP2B)', CHAR(13), ''),CHAR(10),''),',','')) KET,
RTRIM('') NAMAREKANAN
...
SUBSTRING(b.kd_rek5,3,1) AS KDJNSAKUN,(SELECT DISTINCT REPLACE(REPLACE(REPLACE(nm_rek3, CHAR(13), ''),CHAR(10),''),',','') FROM ms_rek3 WHERE kd_rek3=SUBSTRING(b.kd_rek5,1,3)) AS URAIANJNSAKUN,
SUBSTRING(b.kd_rek5,4,2) AS KDOBJAKUN,(SELECT DISTINCT  REPLACE(REPLACE(REPLACE(nm_rek4, CHAR(13), ''),CHAR(10),''),',','') FROM ms_rek4 WHERE kd_rek4=SUBSTRING(b.kd_rek5,1,5)) AS URAIANOBJAKUN,
SUBSTRING(b.kd_rek5,6,2) AS KDRINCIOBJAKUN,(SELECT DISTINCT  REPLACE(REPLACE(REPLACE(nm_rek5, CHAR(13), ''),CHAR(10),''),',','') FROM ms_rek5 WHERE kd_rek5=b.kd_rek5) AS URAIANRINCIOBJAKUN,
...
SELECT left(b.kd_skpd,7)+'.00' as kd_skpd, b.no_sp2d, a.kd_rek5,b.no_nnt,SUM(a.nilai) nil_pot FROM trdstrpot a INNER JOIN trhstrpot b
ON a.no_bukti=b.no_bukti AND a.kd_skpd=b.kd_skpd
WHERE RTRIM(a.kd_rek5) IN $par_rek_pot
GROUP BY left(b.kd_skpd,7),b.no_sp2d,a.kd_rek5,b.no_nnt
)x on x.kd_skpd=z.kd_skpd and rtrim(x.no_sp2d) = rtrim(z.NOSP2D)
group by z.kd_skpd,z.TAHUN,z.KDSATKER,z.KDPEMDA,z.KDURUSAN,z.URAIANURUSAN,
z.KDKELURUSAN,z.URAIANKELURUSAN,z.KDORG,z.URAIANORG,z.NOSP2D,x.kd_rek5,x.nil_pot,x.no_nnt
)x where x.NOSP2D <> '' and x.NILAIPAJAK IS NOT NULL 
";
