...
class Lamp_pmk extends CI_Controller
{
	...
	function penerimaan_skpd() {
        $data['page_title']= 'SKPD PENERIMAAN';
        $this->template->set('title', 'SKPD PENERIMAAN');   
	...
	$cRet .="<TABLE style=\"border-collapse:collapse;font-family:Arial\" width=\"100%\" border=\"0\" cellspacing=\"0\" cellpadding=\"1\" align=\"center\">
                    ...
                    <tr><td align=\"center\" style=\"border-left:hidden;border-bottom:hidden;border-top:hidden\"><b>UPTD PENGELOLAAN KAS DAERAH KABUPATEN SANGGAU </b></tr>
		...
		$sql = "select 'TOTAL PENDAPATAN' as nama, SUM(b.rupiah) as nilai
				...
				ON a.no_kas=b.no_kas AND a.kd_skpd=b.kd_skpd
				WHERE LEFT(kd_rek5,1)='4' AND a.kd_skpd !='4.02.01.00' $where
				UNION ALL
				select 'TOTAL PENDAPATAN PAJAK DAN RETRIBUSI' as nama, SUM(b.rupiah) as nilai
				FROM trhkasin_ppkd a INNER JOIN trdkasin_ppkd b 
				ON a.no_kas=b.no_kas AND a.kd_skpd=b.kd_skpd
				WHERE LEFT(kd_rek5,3) IN ('411','412') AND a.kd_skpd !='4.02.01.00' $where";
		...
		foreach ($hasil->result() as $row)
			...
			$cRet .="<table style=\"border-collapse:collapse;font-family:Arial;font-size:12px\" width=\"100%\" align=\"center\" border=\"0\" cellspacing=\"3\" cellpadding=\"3\">
				...
				$sql = "SELECT a.kode,nama,ISNULL(nilai,0) as nilai FROM (
						SELECT LEFT(kd_rek5,3) as kode, b.nm_rek3 as nama FROM trdrka a 
						INNER JOIN ms_rek3 b ON LEFT(a.kd_rek5,3)=kd_rek3
						WHERE LEFT(kd_rek5,1)='4' AND kd_skpd !='4.02.01.00'
						GROUP BY LEFT(kd_rek5,3), b.nm_rek3) a
						LEFT JOIN 
						(SELECT LEFT(kd_rek5,3) as kode, SUM(rupiah) as nilai FROM trhkasin_ppkd a 
						INNER JOIN trdkasin_ppkd b ON a.no_kas=b.no_kas and a.kd_skpd=b.kd_skpd
						WHERE LEFT(kd_rek5,1)='4' AND a.kd_skpd !='4.02.01.00' $where
						...
						SELECT LEFT(kd_rek5,3)+'.'+kd_skpd as kode, nm_skpd as nama FROM trdrka a 
						WHERE LEFT(kd_rek5,1)='4' AND kd_skpd !='4.02.01.00'
						GROUP BY LEFT(kd_rek5,3), kd_skpd,nm_skpd) a
						LEFT JOIN
						...
						SELECT LEFT(kd_rek5,5)+'.'+kd_skpd kode, b.nm_rek4 as nama FROM trdrka a 
						INNER JOIN ms_rek4 b ON LEFT(a.kd_rek5,5)=kd_rek4
						WHERE LEFT(kd_rek5,1)='4' AND kd_skpd !='4.02.01.00' 
						GROUP BY LEFT(kd_rek5,5),kd_skpd, b.nm_rek4)a
						LEFT JOIN 
			...
			// PPKD
			$sql = "SELECT a.kode,nama, ISNULL(nilai,0) as nilai FROM 
					(SELECT LEFT(kd_rek5,2) as kode, nm_rek2 as nama FROM trdrka a
					INNER JOIN ms_rek2 b ON LEFT(kd_rek5,2)=b.kd_rek2
					WHERE LEFT(kd_rek5,1) IN ('4','6') AND kd_skpd='4.02.01.00' 
					GROUP BY LEFT(kd_rek5,2),nm_rek2) a
					LEFT JOIN 
					(SELECT LEFT(kd_rek5,2) as kode, SUM(rupiah) as nilai FROM trhkasin_ppkd a 
					INNER JOIN trdkasin_ppkd b ON a.no_kas=b.no_kas and a.kd_skpd=b.kd_skpd
					WHERE LEFT(kd_rek5,1) IN ('4','6') AND a.kd_skpd ='4.02.01.00' $where
					...
					SELECT a.kode,nama, ISNULL(nilai,0) as nilai FROM 
					(SELECT LEFT(kd_rek5,3) as kode, nm_rek3 as nama FROM trdrka a
					INNER JOIN ms_rek3 b ON LEFT(kd_rek5,3)=b.kd_rek3
					WHERE LEFT(kd_rek5,1) IN ('4','6')  AND kd_skpd='4.02.01.00' 
					GROUP BY LEFT(kd_rek5,3),nm_rek3) a
					LEFT JOIN 
					(SELECT LEFT(kd_rek5,3) as kode, SUM(rupiah) as nilai FROM trhkasin_ppkd a 
					INNER JOIN trdkasin_ppkd b ON a.no_kas=b.no_kas and a.kd_skpd=b.kd_skpd
					WHERE LEFT(kd_rek5,1) IN ('4','6')  AND a.kd_skpd ='4.02.01.00' $where
					...
					SELECT a.kode,nama, ISNULL(nilai,0) as nilai FROM 
					(SELECT LEFT(kd_rek5,7) as kode, nm_rek5 as nama FROM trdrka a
					WHERE LEFT(kd_rek5,1) IN ('4','6')  AND kd_skpd='4.02.01.00' 
					GROUP BY LEFT(kd_rek5,7),nm_rek5) a
					LEFT JOIN 
		...
		foreach ($hasil->result() as $row)
			...
			$sql = "select SUM(b.rupiah) as nilai
				FROM trhkasin_ppkd a INNER JOIN trdkasin_ppkd b 
				ON a.no_kas=b.no_kas AND a.kd_skpd=b.kd_skpd
				WHERE LEFT(kd_rek5,1)='5' AND a.kd_skpd='4.02.01.00' $where";
