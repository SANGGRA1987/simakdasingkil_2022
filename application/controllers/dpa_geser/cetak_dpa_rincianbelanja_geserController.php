...
class cetak_dpa_rincianbelanja_geserController extends CI_Controller {
    ...
    function __construct(){  
       ...
       function rincianbelanja_geser(){
        if($this->session->userdata('type')=='1'){
            $data['jenis']="DPA";
            $data['page_title']= 'CETAK DPA ';
            $this->template->set('title', 'CETAK DPA ');   
            $this->template->load('template','anggaran/dpa/pergeseran/list_cetak_skpd',$data) ;
        }else{
    ...
    function list_cetakDPA_belanja_rinci_pergeseran($skpd=''){
        ...
        $data['page_title']= 'CETAK DPA ';
        $this->template->set('title', 'CETAK DPA ');   
        $this->template->load('template','anggaran/dpa/pergeseran/dpa_skpd_rincian_belanja',$data) ;
    }
...
function preview_rincian_belanja_skpd_pergeseran(){
           ...
           $sqlttd1="SELECT nama as nm,nip as nip,jabatan as jab, pangkat as pangkat FROM ms_ttd WHERE (REPLACE(id, ' ', 'a')='$ttd1' )  ";
            ...
            $sqlttd2="SELECT nama as nm,nip as nip,jabatan as jab, pangkat as pangkat FROM ms_ttd WHERE (REPLACE(id, ' ', 'a')='$ttd2')  ";
                        ...
                        $cRet .= "<table style='border-collapse:collapse;font-size:10px' width='100%' align='center' border='1' cellspacing='2' cellpadding='5'>
                            ...
                            $sql1="SELECT * FROM(SELECT 0 header,0 no_po, LEFT(a.kd_rek6,1)AS rek1,LEFT(a.kd_rek6,1)AS rek,b.nm_rek1 AS nama, '' spek, '' as koefisien,0 AS volume,' 'AS satuan, 0 AS harga, '' spek2, '' as koefisien2,0 AS volume2,' 'AS satuan2, 0 AS harga2,
                                    ...
                                    SUBSTRING(a.no_trdrka,24,15)='$sub' GROUP BY RIGHT(a.no_trdrka,12),b.header, b.no_po,b.uraian)z WHERE header='1' 
                                        ...
                                        UNION ALL
                                        SELECT a. header,a.no_po as no_po,RIGHT(a.no_trdrka,12) AS rek1,' 'AS rek,a.uraian AS nama,spesifikasi$status_speksifikasi1 as spek, koefisien$koefisien1 koefisien1, a.volume$status_volume1 AS volume,a.satuan$status_satuan1 AS satuan, a.harga$status_harga1 AS harga, a.spesifikasi$status_speksifikasi2 as spek, $koefisien3 koefisien2, a.volume$status_volume2 AS volume,a.satuan$status_satuan2 AS satuan, harga$status_harga2 AS harga,
                    ...
                    $nilangsub2=0;
                            ...
                            $cRet    .=" 
                                        <tr>                                    
                                         ...
                                         <td style='vertical-align:top;'  align='right'>".number_format($nilangsub,2,',','.')."</td>
                                         <td colspan='4' align='right' style='vertical-align:top;' >Jumlah Anggaran Sub Kegiatan Setelah $nama_status</td>
                                         <td style='vertical-align:top;'  align='right'>".number_format($nilangsub2,2,',','.')."</td>
 ...
 if($ttd1!='tanpa'){
            ...
            $tambahan="";
                ...
                $angkas5=$this->db->query("SELECT  kd_skpd, 
                                                ...
                                                select bulan, left(kd_gabungan,22) kd_skpd , sum(nilai$status_anggaran2) nilai from trdskpd_ro WHERE left(kd_rek6,1)='5' and kd_subkegiatan='$sub' GROUP BY bulan, left(kd_gabungan,22)
