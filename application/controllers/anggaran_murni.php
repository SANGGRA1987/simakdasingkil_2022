...
public $ppkd1_lama = "4.02.02.02";
    ...
    {    
        parent::__construct();
		$this->load->model('anggaran_murni_model', 'anggaran_murni_model');
    }
  ...
  function simpan_rincian_dpo(){
        ...
        $query1 = $this->db->query("UPDATE trskpd set 
            tk_mas          =(select 'Rp. '+(SELECT CAST(CONVERT(VARCHAR, CAST(sum(nilai) AS MONEY), 1) AS VARCHAR)) as jum from trdrka where left(kd_sub_kegiatan,12)=left('$kd_kegiatan',12) and left(kd_skpd,20)=left('$no_trdrka',20)),
            tk_mas_sempurna =(select 'Rp. '+(SELECT CAST(CONVERT(VARCHAR, CAST(sum(nilai) AS MONEY), 1) AS VARCHAR)) as jum from trdrka where left(kd_sub_kegiatan,12)=left('$kd_kegiatan',12) and left(kd_skpd,20)=left('$no_trdrka',20)),
            tk_mas_ubah     =(select 'Rp. '+(SELECT CAST(CONVERT(VARCHAR, CAST(sum(nilai) AS MONEY), 1) AS VARCHAR)) as jum from trdrka where left(kd_sub_kegiatan,12)=left('$kd_kegiatan',12) and left(kd_skpd,20)=left('$no_trdrka',20))
            where kd_kegiatan=left('$kd_kegiatan',12) and left(kd_skpd,20)=left('$no_trdrka',20)");  

        echo $query1 = $this->db->query("UPDATE trskpd set 
            total           = (select sum(nilai) as jum from trdrka where kd_sub_kegiatan='$kd_kegiatan' and left(kd_skpd,20)=left('$no_trdrka',20)),
            total_sempurna  = (select sum(nilai_sempurna) as jum from trdrka where kd_sub_kegiatan='$kd_kegiatan' and left(kd_skpd,20)=left('$no_trdrka',20)),
            total_ubah      = (select sum(nilai_ubah) as jum from trdrka where kd_sub_kegiatan='$kd_kegiatan' and left(kd_skpd,20)=left('$no_trdrka',20)),
            username1       = '$id',last_update=getdate() where kd_sub_kegiatan='$kd_kegiatan' and left(kd_skpd,20)=left('$no_trdrka',20)"); 
   ...
   function hapus_rincian_dpo(){
        ...
        $query1 = $this->db->query("UPDATE trskpd set 
            tk_mas          =(select 'Rp. '+(SELECT CAST(CONVERT(VARCHAR, CAST(sum(nilai) AS MONEY), 1) AS VARCHAR)) as jum from trdrka where left(kd_sub_kegiatan,12)=left('$kd_kegiatan',12) and left(kd_skpd,20)=left('$norka',20)),
            tk_mas_sempurna =(select 'Rp. '+(SELECT CAST(CONVERT(VARCHAR, CAST(sum(nilai) AS MONEY), 1) AS VARCHAR)) as jum from trdrka where left(kd_sub_kegiatan,12)=left('$kd_kegiatan',12) and left(kd_skpd,20)=left('$norka',20)),
            tk_mas_ubah     =(select 'Rp. '+(SELECT CAST(CONVERT(VARCHAR, CAST(sum(nilai) AS MONEY), 1) AS VARCHAR)) as jum from trdrka where left(kd_sub_kegiatan,12)=left('$kd_kegiatan',12) and left(kd_skpd,20)=left('$norka',20))
            where kd_kegiatan=left('$kd_kegiatan',12) and left(kd_skpd,20)=left('$norka',20)");  
    
        echo $query1 = $this->db->query("
            UPDATE trskpd set
            total           = (select isnull(sum(nilai),0) as jum from trdrka where kd_sub_kegiatan='$kd_kegiatan' and left(kd_skpd,20)=left('$skpd',20)), 
            total_ubah      = (select isnull(sum(nilai_ubah),0) as jum from trdrka where kd_sub_kegiatan='$kd_kegiatan' and left(kd_skpd,20)=left('$skpd',20)),
            total_sempurna  = (select isnull(sum(nilai_sempurna),0) as jum from trdrka where kd_sub_kegiatan='$kd_kegiatan' and left(kd_skpd,20)=left('$skpd',20)),
            username1       = '$id',last_update=getdate() where kd_sub_kegiatan='$kd_kegiatan' and left(kd_skpd,20)=left('$skpd',20) ");    
