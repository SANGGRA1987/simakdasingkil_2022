...
//end buka_kunci renja
    ...
    function tambah_user()
    {
        ...
        else
        {
            ...
            $data = array(                      
                        'id_user' => $this->db->query("SELECT max(CAST(id_user AS INT))+1 hasil FROM [user]")->row('hasil'),
                        'user_name' => $this->input->post('user_name'),
                        'password' => md5($this->input->post('password')),
                        'kd_skpd' => $this->input->post('pcskpd'),
                        'type' => $this->input->post('type'),
...
//                      'id_user' => $this->input->post('id_user'),
    ...
    function ambil_burusan() {
        $lccr = $this->input->post('q');
        $sql = "SELECT replace(kd_bidang_urusan,'.','-') kd_bidang_urusan, nm_bidang_urusan FROM ms_bidang_urusan where upper(kd_bidang_urusan) like upper('%$lccr%') or upper(nm_bidang_urusan) like upper('%$lccr%') ";
        $query1 = $this->db->query($sql);  
        $result = array();
        $ii = 0;
        foreach($query1->result_array() as $resulte)
...
inner join ms_rek6 b on a.kd_rek5=b.kd_rek5 where (upper(a.kd_rek5) like upper('%$lccr%') or upper(a.nm_rek5) like upper('%$lccr%')) ";
    ...
    function load_bank() {
     ...
     function load_ttd() {
        ...
        $sql = "SELECT top $rows * from ms_ttd where $where  nip not in (SELECT TOP $offset  nip from  
                ms_ttd $where3 order by id,nama ) order by id,nama  ";//limit $offset,$rows";
        $query1 = $this->db->query($sql);  
        $result = array();
        $ii = 0;
        foreach($query1->result_array() as $resulte)
    ...
    function simpan_skpd(){
       ...
       function simpan_master(){
        ...
        $bid=$this->input->post('bid');
            ...
            $sql="select max(id)+1 oke from ms_ttd";
            $asg = $this->db->query($sql)->row();
            $id_ttd=$asg->oke;

            $sql = "SET IDENTITY_INSERT ms_ttd ON
			       insert into ms_ttd (nip,nama,jabatan,pangkat,kd_skpd,kode,bidang,id) 
                values ('$nip','$nama','$jab','$pang','$dinas','$kode','$bid','$id_ttd')";
            $asg = $this->db->query($sql);
            if($asg){
                echo '2';
            }else{
    ...
    function load_daftar_harga_detail_ck($norek='') {
        ...
        $sql    = "SELECT *, kd_rek6 as ck from trdharga where kd_rek6 = '$norek' order by no_urut ";
        $query1 = $this->db->query($sql);  
        $result = array();
        $ii     = 0;
        foreach($query1->result_array() as $resulte)
...
upper('%$kriteria%') or kd_sumber_dana1 like'%$kriteria%'))  order by kd_sumber_dana1";
    ...
    function ambil_rekening5_all_ar() {
        ...
        if ( $notin <> ''){
            $where = " and kd_rek6 not in ($notin) ";
        } else {
            $where = " ";
        }
        
        if ( $jnskegi =='4' ) {
            $sql = "SELECT kd_rek6, nm_rek6 FROM ms_rek6 where (left(kd_rek6,1)='4' )
                    and (upper(kd_rek6) like upper('%$lccr%') or upper(nm_rek6) like upper('%$lccr%')) $where order by kd_rek6";
        }elseif ( $jnskegi=='5'){
            $sql = "SELECT kd_rek6, nm_rek6 FROM ms_rek6 where (left(kd_rek6,1)='5')
                and (upper(kd_rek6) like upper('%$lccr%') or upper(nm_rek6) like upper('%$lccr%')) $where order by kd_rek6"; 
        } else {
             $sql = "SELECT kd_rek6, nm_rek6 FROM ms_rek6 where (left(kd_rek6,1)='6')
                and (upper(kd_rek6) like upper('%$lccr%') or upper(nm_rek6) like upper('%$lccr%')) $where order by kd_rek6";
        }
...
//=================================================================================
   ...
   function mperusahaan()
    ...
    function ambil_bank() {
        $lccr = $this->input->post('q');
        $sql = "SELECT kode, nama FROM ms_bank where upper(kode) like upper('%$lccr%') or upper(nama) like upper('%$lccr%') ";
        $query1 = $this->db->query($sql);  
        $result = array();
        $ii = 0;
        foreach($query1->result_array() as $resulte)
    ...
    function no_urut()
    {
        $sql = "SELECT MAX(kode) as kode FROM ms_perusahaan";
        $query = $this->db->query($sql);
        foreach ($query->result() as $urut) {
            $no = $urut->kode;
        }

        $urut = $no + 1 ;
        $noUrut = str_pad($urut,5,"0",STR_PAD_LEFT);
        $nomor = array('no_urutan' => $noUrut);
    ...
    function load_perusahaan() {
        ...
        $where ='';
        if ($kriteria <> ''){                               
            $where="where (upper(a.nama) like upper('%$kriteria%') or a.kode like'%$kriteria%')";            
        }
        
        $sql = "SELECT a.* from ms_perusahaan a $where order by a.kode";
        $query1 = $this->db->query($sql);  
        $result = array();
        $ii = 0;
        foreach($query1->result_array() as $resulte)
        { 
            ...
            $result[] = array(
                        'id' => $ii,        
                        'kd_perusahaan' => $resulte['kode'],
                        'nama_perusahaan' => $resulte['nama'],
    ...
    function hapus_master_perusahaan(){
        $ctabel = $this->input->post('tabel');
        $cid = $this->input->post('cid');
        $cnid = $this->input->post('cnid');
    ...
    function mkontrak()
    {
        $data['page_title']= 'Master Kontrak';
        $this->template->set('title', 'Master Kontrak');   
        $this->template->load('template','master/kontrak/mkontrak',$data) ;
    }
    ...
    function load_kontrak() {
        ...
        $sql = "SELECT a.*,b.*,c.nm_skpd from ms_kontrak a inner join ms_perusahaan b on a.kd_perusahaan=b.kode inner join ms_skpd c on b.kd_skpd=c.kd_skpd order by a.no_kontrak";
        $query1 = $this->db->query($sql);  
        $result = array();
        $ii = 0;
        foreach($query1->result_array() as $resulte)
        { 
            ...
            $result[] = array(
                        ...
                        'nama_proyek' => $resulte['nama_proyek'],
    ...
    function ambil_perusahaan() {
        ...
        $sql = "SELECT a.*,b.nm_skpd from ms_perusahaan a inner join ms_skpd b on a.kd_skpd=b.kd_skpd $where order by kode";
        $query1 = $this->db->query($sql);  
        $result = array();
        $ii = 0;
        foreach($query1->result_array() as $resulte)
    ...
    function update_master_kontrak(){
         $query = $this->input->post('st_query');
        $asg = $this->db->query($query);
        if($asg){
            echo '1';
    ...
    function hapus_master_kontrak(){
        ...
        $cid = $this->input->post('cid');
        $cnid = $this->input->post('cnid');
        $cnkd = $this->input->post('cnkd');
        $ckd = $this->input->post('ckd');
        
        $csql = "delete from $ctabel where $cid = '$cnid' and $ckd ='$cnkd'";
    ...
    function ambil_kegiatan_kontrak() {
        $lccr = $this->input->post('q');
        $lc_kdskpd = $this->input->post('lc_kdskpd');
        $sql = "SELECT kd_subkegiatan, nm_subkegiatan FROM trskpd where kd_skpd='$lc_kdskpd' 
                and upper(kd_subkegiatan) like upper('%$lccr%') or nm_subkegiatan like '%$lccr%' order by kd_subkegiatan";
        //$sql = "SELECT kd_subkegiatan, nm_subkegiatan FROM trskpd where kd_skpd='$lc_kdskpd' order by kd_subkegiatan";
        $query1 = $this->db->query($sql);  
        $result = array();
        $ii = 0;
        foreach($query1->result_array() as $resulte)
