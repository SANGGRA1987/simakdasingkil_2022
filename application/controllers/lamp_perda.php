...
class Lamp_perda extends CI_Controller
{
	...
	function perda_lampII_10(){
		...
		$sql = "SELECT * FROM ms_ttd WHERE kode='PA' and kd_skpd= '4.02.01.00' AND (UPPER(kode) LIKE UPPER('%$lccr%') OR UPPER(nama) LIKE UPPER('%$lccr%'))";   
...
<td width=\"15%\" align=\"center\" bgcolor=\"#CCCCCC\" style=\"font-size:12px\"><b>UNIT PENGOLAHAN DATA ELEKTRONIK DISHUBKOMINFO KABUPATEN SANGGAU</b></td>
			...
			$sql = "SELECT uraian, seq, cetak, bold,parent as par, isnull(kode_1,'-') as rek, isnull(kode_2,'-') as kode_2, isnull(kode_3,'-') as kode_3 FROM map_lra_prov ORDER BY kode
                    ...
                    foreach ($hasil->result() as $row)
                    {
					   ...
					   $sql = "SELECT 
						...
						,SUM(CASE WHEN a.kd_unit='1.20.08.01' THEN ($cetak) ELSE 0 END) AS a1200801
						,SUM(CASE WHEN a.kd_unit='1.20.08.02' THEN ($cetak) ELSE 0 END) AS a1200802
						,SUM(CASE WHEN a.kd_unit='1.20.08.03' THEN ($cetak) ELSE 0 END) AS a1200803
						,SUM(CASE WHEN a.kd_unit='1.20.08.04' THEN ($cetak) ELSE 0 END) AS a1200804
						,SUM(CASE WHEN a.kd_unit='1.20.08.05' THEN ($cetak) ELSE 0 END) AS a1200805
						,SUM(CASE WHEN a.kd_unit='1.20.08.06' THEN ($cetak) ELSE 0 END) AS a1200806
						,SUM(CASE WHEN a.kd_unit='1.20.08.07' THEN ($cetak) ELSE 0 END) AS a1200807
						,SUM(CASE WHEN a.kd_unit='1.20.08.08' THEN ($cetak) ELSE 0 END) AS a1200808
						,SUM(CASE WHEN a.kd_unit='1.20.08.09' THEN ($cetak) ELSE 0 END) AS a1200809
						,SUM(CASE WHEN a.kd_unit='1.20.08.10' THEN ($cetak) ELSE 0 END) AS a1200810
						,SUM(CASE WHEN a.kd_unit='1.20.08.11' THEN ($cetak) ELSE 0 END) AS a1200811
						,SUM(CASE WHEN a.kd_unit='1.20.08.12' THEN ($cetak) ELSE 0 END) AS a1200812
						,SUM(CASE WHEN a.kd_unit='1.20.08.13' THEN ($cetak) ELSE 0 END) AS a1200813
						,SUM(CASE WHEN a.kd_unit='1.20.08.14' THEN ($cetak) ELSE 0 END) AS a1200814
						,SUM(CASE WHEN a.kd_unit='1.20.08.15' THEN ($cetak) ELSE 0 END) AS a1200815
						,SUM(CASE WHEN a.kd_unit='1.20.08.16' THEN ($cetak) ELSE 0 END) AS a1200816
...
function cetak_akum_susut_jij($kdskpd='',$rek3='',$ctk=''){
		...
		$cRet ='<TABLE style="border-collapse:collapse" width="100%" border="0" cellspacing="0" cellpadding="1" align=center>
					<TR>
						...
						<BR>Pemerintah KABUPATEN SANGGAU Tahun '.$lntahunang.'
	...
	$cRet .="<TABLE style=\"border-collapse:collapse;font-family:Arial\" width=\"100%\" border=\"1\" cellspacing=\"0\" cellpadding=\"1\" align=\"center\">
		...
		$cRet .="<table style=\"border-collapse:collapse;font-family:Arial;font-size:11px\" width=\"100%\" align=\"center\" border=\"1\" cellspacing=\"3\" cellpadding=\"3\">
			...
			$sql = "SELECT SUM(nilai) as nil_ang, SUM(debet-kredit) as nilai FROM data_jurnal(1,1) WHERE (LEFT(kd_rek5,5) 
	...
	if($panjang==10){
		...
		$cRet .="<table style=\"border-collapse:collapse;font-family:Arial;font-size:11px\" width=\"100%\" align=\"center\" border=\"1\" cellspacing=\"3\" cellpadding=\"3\">
			...
			$sql = "SELECT 
					...
					FROM
					(SELECT LEFT(kd_rek5,1) as kd_rek, SUM(nilai_ang) as nilai_ang, SUM(real_spj) as real_spj FROM Data_realisasi_keg4($bulan,$anggaran) WHERE LEFT(kd_rek5,1) IN ('4','5','6') $where
					GROUP BY LEFT(kd_rek5,1)) a;
					";
					  $hasil = $this->db->query($sql);
                    foreach ($hasil->result() as $row)
			...
			$sql = "SELECT 
					...
					FROM
					(SELECT LEFT(kd_rek5,2) as kd_rek, SUM(nilai_ang) as nilai_ang, SUM(real_spj) as real_spj  FROM Data_realisasi_keg4($bulan,$anggaran) WHERE LEFT(kd_rek5,2) IN ('71','72') $where
					GROUP BY LEFT(kd_rek5,2)) a;
					";
					  $hasil = $this->db->query($sql);
                    foreach ($hasil->result() as $row)
			...
			$sql = "SELECT seq, kode, nama, kode1, kode2, kode3,kode4, spasi FROM map_lra_sap ORDER BY seq
					...
					$sql = "SELECT SUM(nilai_ang) as nil_ang, SUM(real_spj) as nilai FROM Data_realisasi_keg4($bulan,$anggaran) WHERE (LEFT(kd_rek5,1) IN ($kode1) or LEFT(kd_rek5,2) IN ($kode2) or LEFT(kd_rek5,3) IN ($kode3) or LEFT(kd_rek5,5) IN($kode4)) $where
					";
	...
	if($panjang==10){
		...
		$cRet .="<table style=\"border-collapse:collapse;font-family:Arial;font-size:11px\" width=\"100%\" align=\"center\" border=\"1\" cellspacing=\"3\" cellpadding=\"3\">
			...
			$sql = "SELECT 
					...
					FROM
					(SELECT LEFT(kd_ang,1) as kd_rek, SUM(nilai_ang) as nil_ang, SUM(real_spj) as real_spj FROM Data_realisasi_keg4($bulan,$anggaran) WHERE LEFT(kd_ang,1) IN ('4','5') $where
			...
			$sql = "SELECT 
					...
					FROM
					(SELECT LEFT(kd_ang,2) as kd_rek, SUM(nilai_ang) as nil_ang, SUM(real_spj) as real_spj FROM Data_realisasi_keg4($bulan,$anggaran) WHERE LEFT(kd_ang,2) IN ('61','62') $where
			...
			$sql = "SELECT seq, kode, nama, kode1, kode2, kode3,kode4, spasi FROM map_lra_permen ORDER BY seq
					...
					$sql = "SELECT SUM(nilai_ang) as nil_ang, SUM(real_spj) as nilai FROM Data_realisasi_keg4($bulan,$anggaran) WHERE (LEFT(kd_ang,1) IN ($kode1) or LEFT(kd_ang,2) IN ($kode2) or LEFT(kd_ang,3) IN ($kode3) or LEFT(kd_ang,5) IN($kode4)) $where
					";
...
ON a.kode=b.kode
	...
	$cRet .="<TABLE style=\"border-collapse:collapse;font-family:Arial\" width=\"100%\" border=\"1\" cellspacing=\"0\" cellpadding=\"1\" align=\"center\">
		...
		$cRet .="<table style=\"border-collapse:collapse;font-family:Arial;font-size:11px\" width=\"100%\" align=\"center\" border=\"1\" cellspacing=\"3\" cellpadding=\"3\">
				...
				$sql="SELECT kd_kegiatan,kd_rek,nm_rek,anggaran,sd_bulan_ini FROM realisasi_jurnal_pend($bulan,$anggaran) $where $where1 ORDER BY kd_kegiatan,kd_rek";
					...
					$sql="SELECT kd_kegiatan,kd_rek,nm_rek,nil_ang anggaran,nilai sd_bulan_ini FROM $tabel($bulan,$anggaran) $where $where1 
					...
					$sql="SELECT '1' kd_rek,'BELANJA LANGSUNG' nm_rek,SUM(nil_ang) anggaran,SUM(nilai) sd_bulan_ini
						  FROM $tabel($bulan,$anggaran) $where 
							AND kd_rek IN('521','522','523')
							UNION ALL
	...
	function cetak_lamp_semester_rinci_org_spj($bulan='',$ctk='',$anggaran='',$kd_skpd='',$jenis='',$tglttd='', $ttd='',$spj=''){
		...
		$where= "WHERE kd_skpd='$kd_skpd'";	
		$tanggal = $tglttd == '-' ? '' : 'Singkil, '.$this->tukd_model->tanggal_format_indonesia($tglttd) ;
	if($ttd=='-'){
		$nama='';
		$pangkat='';
		$jabatan='';
	...
	$cRet .="<TABLE style=\"border-collapse:collapse;font-family:Arial\" width=\"100%\" border=\"1\" cellspacing=\"0\" cellpadding=\"1\" align=\"center\">
		...
		$cRet .="<table style=\"border-collapse:collapse;font-family:Arial;font-size:11px\" width=\"100%\" align=\"center\" border=\"1\" cellspacing=\"3\" cellpadding=\"3\">
				...
				$sql="SELECT RIGHT(kd_kegiatan,6) kd_kegiatan,kd_rek,nm_rek,SUM(anggaran) anggaran, SUM(sd_bulan_ini) sd_bulan_ini 
					 FROM realisasi_jurnal_pend($bulan,$anggaran) $where $where1  
					 GROUP BY RIGHT(kd_kegiatan,6),kd_rek,nm_rek
					...
					$sql="SELECT SUBSTRING(kd_kegiatan,17,6) kd_kegiatan,kd_rek,nm_rek,SUM(anggaran) anggaran,SUM(sd_bulan_ini) sd_bulan_ini 
					      FROM realisasi_jurnal_rinci($bulan,$anggaran) $where $where1
						  AND SUBSTRING(kd_kegiatan,17,2)='00'
						  GROUP BY SUBSTRING(kd_kegiatan,17,6),kd_rek,nm_rek
						  ORDER BY kd_kegiatan,kd_rek";
					$hasil = $this->db->query($sql);
                    foreach ($hasil->result() as $row)
                    {
	...
	$ttd=str_replace("abc"," ",$ttd);
	$sqlsc="SELECT nama,jabatan,pangkat FROM ms_ttd where kode='PA' and kd_skpd='1.20.00.00'";
                 $sqlsclient=$this->db->query($sqlsc);
                 foreach ($sqlsclient->result() as $rowttd)
                {
                    $nama = $rowttd->nama;
	...
	$cRet .="<TABLE style=\"border-collapse:collapse;font-family:Arial\" width=\"100%\" border=\"1\" cellspacing=\"0\" cellpadding=\"1\" align=\"center\">
		...
		$cRet .="<table style=\"border-collapse:collapse;font-family:Arial;font-size:11px\" width=\"100%\" align=\"center\" border=\"1\" cellspacing=\"3\" cellpadding=\"3\">
				...
				$sql="SELECT RIGHT(kd_kegiatan,5) kd_kegiatan,kd_rek,nm_rek, SUM(anggaran) anggaran, SUM(sd_bulan_ini) sd_bulan_ini 
					...
					$sql="SELECT SUBSTRING(kd_kegiatan,17,5) kd_kegiatan,kd_rek,nm_rek,SUM(nil_ang) anggaran,SUM(nilai) sd_bulan_ini FROM $tabel($bulan,$anggaran) $where $where1 
